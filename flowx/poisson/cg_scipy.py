...
def solve_cg_scipy(grid,ivar,rvar,maxiter=3000,tol=1e-9,verbose=False):
    ...
    xs,it,err_list = solve_sparse(A,b,asol,nx,ny)
...
def solve_sparse(A,b,asol,nx,ny):
    num_iters = 0
    err_list = []
    ...
    def callback(xk):
        ...
        err = (np.sqrt(np.sum((asol[1:-1,1:-1] - xn)**2) /
              ((nx + 2) * (ny + 2))))
  
        err_list.append(err)

    x = sps.linalg.cg(A, b, tol=1e-13,maxiter=10000, callback=callback)[0]
    return x,num_iters,err_list
